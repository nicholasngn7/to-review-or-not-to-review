...
> `docs/assets/videos/`. For **exact-version** v0.1/v0.2 videos, run this harness
> against a historical app started from a `v0.1.0`/`v0.2.0` worktree via `DEMO_BASE_URL`
> (old tags lack the `demo:*` scripts) — see
> [`../frontend/demo/README.md`](../frontend/demo/README.md). The v0.3 import video is a
> **local fixture-based demo**, not live GitHub/GitLab integration. `ffmpeg` conversion
> to `.mp4`/`.gif` is optional. This hand script remains useful for a narrated recording.
